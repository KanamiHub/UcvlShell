...
def downloadFile(downloader,filename,currentBits,totalBits,speed,time,args):
    ...
    pass
def uploadFile(filename,update, bot, message, thread=None, proxy="",zips=False):
				...
				if "login_csrf" in s.text:
					...
					if "caducado" in post.text:
						...
						if filename in briefcase.text:						      				
						      				bot.editMessageText(message,filename+' subido')
						else:
							bot.editMessageText(message, 'No subio')
#TechDev @techdev_pro
def processUploadFiles(filename, update,bot,message,thread=None,jdb=None,zips=''):
        err = None
        ...
        if exists(str(update.message.chat.id)+"proxy"):
        	...
        	uploadFile(filename, update, bot, message, thread, proxy='',zips=zips)
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if os.path.getsize(file) > max_file_size:
        ...
        while exists(zipname+".7z.00"+str(current)):
        	...
        	processUploadFiles(name,update,bot,message,thread=None,zips=True)
        	current += 1
        	time.sleep(1)
        try:
            os.unlink(name)
        ...
        print("No zip")
        processUploadFiles(name,file_size,[name],update,bot,message,zips=False)
        file_upload_count = 1
    evidname = ''
    files = []
...
def onmessage(update,bot:TechDevClient):
        ...
        if '/start' in msgText:
            if not exists(str(update.message.chat.id)+'proxy'):
            	...
            	proxy.close()
            proxy = open(str(update.message.chat.id)+'proxy','r')
            if not exists(str(update.message.chat.id)+'username'):
            	username = open(str(update.message.chat.id)+'username','w')
            	username.write("Nada ._.")
            	username.close()
            if not exists(str(update.message.chat.id)+'password'):
